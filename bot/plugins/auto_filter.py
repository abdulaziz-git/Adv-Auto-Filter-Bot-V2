...
async def auto_filter(bot, update):
    ...
    if filters:
        ...
        await bot.send_message(
                ...
                text=f"Pencarian untuk <code>{query}</code> tidak ditemukan, silahkan cari film/TV Series sesuai dengan nama aslinya, contoh <code>Free Guy</code>",
                parse_mode="html",
    ...
    if len(results) == 0: # double check
        ...
        try:
            await bot.send_message(
                chat_id = update.chat.id,
                text=f"Hasil untuk pencarian <code>{query}</code> ditemukan {(len_results)} file",
